...
export function useSessionManager() {
  ...
  const requestsEnabled = activeSession?.requestsEnabled !== false;
  ...
  const feedAspectRatio = workingSession?.feedAspectRatio ?? '16:9';
  const feedTemplateId = workingSession?.feedTemplateId ?? 'default';
  ...
  async function toggleRequestsEnabled() {
    ...
    await fetch(`/api/dj/sessions/${activeSession._id}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ requestsEnabled: !requestsEnabled }),
    });
    mutateSessions();
  }
  ...
  async function setFeedAspectRatio(ratio) {
    ...
    await fetch(`/api/dj/sessions/${workingSession._id}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
